...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!agreed) { setError("You must agree to the Terms & Policies."); return; }
    if (password.length < 6) { setError("Password must be at least 6 characters."); return; }
    setLoading(true);
    try { await signup(email, password, name); } catch { setError("Signup failed."); }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground font-serif">Create your client account</p>
        </div>

        <Card className="shadow-card">
          <CardHeader className="pb-4">
            <CardTitle className="font-display text-xl">Sign Up</CardTitle>
            <CardDescription>Get started with your cleaning portal</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Button variant="outline" onClick={loginWithGoogle} className="w-full font-serif">
              ...
              <Button variant="outline" onClick={loginWithApple} className="w-full font-serif">
                <svg className="h-4 w-4 mr-2" viewBox="0 0 24 24" fill="currentColor"><path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.53-3.23 0-1.44.62-2.2.44-3.06-.4C3.79 16.17 4.36 9.02 8.73 8.76c1.23.06 2.09.7 2.81.73.96-.19 1.88-.87 2.9-.78 1.23.11 2.16.67 2.78 1.7-2.56 1.53-1.95 4.89.59 5.82-.47 1.24-.84 2.06-1.76 4.05zM12.04 8.68c-.14-2.33 1.82-4.3 4.01-4.48.28 2.55-2.31 4.65-4.01 4.48z"/></svg>
                Apple
              </Button>
            </div>
